...
import { apiSuccess, apiUnauthorized, apiRateLimited, handleApiError } from '@/lib/api/standardResponse'
...
describe('Services API - POST', () => {
  beforeEach(() => {
    ...
    ;(apiSuccess as jest.Mock).mockImplementation((data, options) => ({ data, options, status: 201 }))
    ...
    ;(handleApiError as jest.Mock).mockImplementation((error) => ({ error: error.message, status: 500 }))
  })
  ...
  it('returns unauthorized for unauthenticated user', async () => {
    ...
    const mockRequest = {
      json: jest.fn().mockResolvedValue({ title: 'Test', category: 'Other', fixed_price_sats: 1000 })
    } as any

    const mockCtx = { body: { title: 'Test', category: 'Other', fixed_price_sats: 1000 } }
  ...
  it('returns rate limited when user exceeds rate limit', async () => {
    ...
    expect(rateLimitWrite).toHaveBeenCalledWith('user-123')
    expect(apiRateLimited).toHaveBeenCalledWith('Too many service creation requests. Please slow down.', expect.any(Number))
    expect(createService).not.toHaveBeenCalled()
  })
  ...
  it('handles service creation errors', async () => {
    ...
    const inputData = { title: 'Test', category: 'Other', fixed_price_sats: 1000 }
    const processedData = {
      title: 'Test',
      category: 'Other',
