...
const { data, error } = await supabase.from('projects').insert([fundingPageData]).select().single();
...
```typescript
// ❌ BAD: Direct access in component
const { data: followers } = await supabase.from('follows').select('*').eq('followed_id', userId);
