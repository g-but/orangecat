...
export function table<T extends keyof Database['public']['Tables']>(name: T): TypedTable<T> {
...
export class DatabaseClient {
  ...
  get profiles() {
    return new ProfilesTable(this.supabase);
  }
  get projects() {
    return new ProjectsTable(this.supabase);
  }
  // ...
}
...
class TypedTable<T extends TableName> {
  ...
  // Type-safe select
  select<K extends keyof TableRow<T>>(columns: K[]): TypedQueryBuilder<T, K> {
...
const { data, cursor } = await db.projects.paginate({ limit: 20, cursor });
```
