...
class GitHubAuth {
  ...
  async updateEnvFile(token) {
    ...
    // Remove existing GITHUB_TOKEN lines (preserve other content)
    const lines = envContent.split('\n');
    const filteredLines = lines.filter(line => {
      const trimmed = line.trim();
      return !trimmed.startsWith('GITHUB_TOKEN=');
    });
    ...
    // Add GitHub token section if not present
    if (!envContent.includes('# GitHub Configuration')) {
      filteredLines.push('');
      filteredLines.push('# GitHub Configuration');
    }
    ...
    console.log('✅ GitHub token saved to .env.local (preserved existing content)');
  }
