-- Migration: Create causes table
-- Date: 2025-01-31
-- Purpose: Create the causes table for charitable fundraising campaigns
...
CREATE TABLE IF NOT EXISTS public.causes (
    ...
    title text NOT NULL,
    description text,
    cause_category_id uuid,
    target_amount numeric,
    current_amount numeric DEFAULT 0,
    currency text DEFAULT 'CHF' CHECK (currency IN ('CHF', 'EUR', 'USD', 'GBP', 'BTC', 'SATS')),
    status text DEFAULT 'draft' CHECK (status IN ('draft', 'active', 'completed', 'cancelled')),
    is_public boolean DEFAULT true,
    is_negotiable boolean DEFAULT true,
    minimum_donation numeric DEFAULT 0,
    end_date date,
    images text[] DEFAULT '{}',
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_causes_user_id ON public.causes(user_id);
CREATE INDEX IF NOT EXISTS idx_causes_status ON public.causes(status);
CREATE INDEX IF NOT EXISTS idx_causes_category ON public.causes(cause_category_id);
CREATE INDEX IF NOT EXISTS idx_causes_public ON public.causes(is_public) WHERE is_public = true;
...
COMMENT ON COLUMN public.causes.end_date IS 'Optional end date for fundraising campaign';
