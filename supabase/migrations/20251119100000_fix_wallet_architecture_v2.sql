-- Migration: Fix Wallet Architecture v2 (CORRECTED)
-- Priority: P0 - CRITICAL
-- Created: 2025-11-19
-- Purpose: Allow wallets to be shared between profiles and projects, support multiple categories per wallet
-- Version: 2.0 - Fixes referential integrity issues from v1
-- Breaking Change: Yes - requires data migration
-- Estimated Time: 5-10 minutes

-- ============================================================================
-- PHASE 1: Create New Wallet Architecture with Referential Integrity
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.wallet_ownerships (
  ...
  owner_id UUID NOT NULL, -- References profiles.id or projects.id (validated by trigger)
...
CREATE INDEX idx_wallet_ownerships_profile ON public.wallet_ownerships(owner_id, is_active) WHERE owner_type = 'profile';
CREATE INDEX idx_wallet_ownerships_project ON public.wallet_ownerships(owner_id, is_active) WHERE owner_type = 'project';

COMMENT ON TABLE public.wallet_ownerships IS 'Many-to-many: which entities can use which wallets';
COMMENT ON COLUMN public.wallet_ownerships.owner_id IS 'References profiles.id or projects.id - validated by trigger';
...
CREATE TABLE IF NOT EXISTS public.wallet_categories (
  ...
  entity_id UUID NOT NULL, -- References profiles.id or projects.id (validated by trigger)
...
COMMENT ON COLUMN public.wallet_categories.entity_id IS 'References profiles.id or projects.id - validated by trigger';

-- ============================================================================
-- PHASE 2: Referential Integrity Triggers (CRITICAL FIX)
-- ============================================================================

-- Trigger 1: Validate wallet_ownerships.owner_id references exist
...
CREATE TRIGGER validate_wallet_ownership_trigger
  BEFORE INSERT OR UPDATE ON public.wallet_ownerships
  FOR EACH ROW EXECUTE FUNCTION validate_wallet_ownership();

COMMENT ON FUNCTION validate_wallet_ownership IS 'Ensures owner_id references valid profile or project';

-- Trigger 2: Cleanup wallet_ownerships when profile deleted
CREATE OR REPLACE FUNCTION cleanup_wallet_ownerships_on_entity_delete()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER cleanup_wallet_ownerships_on_project_delete
  ...
  FOR EACH ROW EXECUTE FUNCTION cleanup_wallet_ownerships_on_entity_delete('project');

COMMENT ON FUNCTION cleanup_wallet_ownerships_on_entity_delete IS 'Cascades deletes from profiles/projects to wallet_ownerships';

-- Trigger 3: Validate wallet_categories.entity_id references exist
CREATE OR REPLACE FUNCTION validate_wallet_category()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER validate_wallet_category_trigger
  BEFORE INSERT OR UPDATE ON public.wallet_categories
  FOR EACH ROW EXECUTE FUNCTION validate_wallet_category();

COMMENT ON FUNCTION validate_wallet_category IS 'Ensures entity_id references valid profile or project';

-- Trigger 4: Cleanup wallet_categories when entity deleted
CREATE OR REPLACE FUNCTION cleanup_wallet_categories_on_entity_delete()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER cleanup_wallet_categories_on_profile_delete
  AFTER DELETE ON public.profiles
  FOR EACH ROW EXECUTE FUNCTION cleanup_wallet_categories_on_entity_delete('profile');

CREATE TRIGGER cleanup_wallet_categories_on_project_delete
  AFTER DELETE ON public.projects
  FOR EACH ROW EXECUTE FUNCTION cleanup_wallet_categories_on_entity_delete('project');

COMMENT ON FUNCTION cleanup_wallet_categories_on_entity_delete IS 'Cascades deletes from profiles/projects to wallet_categories';

-- ============================================================================
-- PHASE 3: Standard Triggers
-- ============================================================================
...
-- Update timestamp trigger for wallet_ownerships
CREATE OR REPLACE FUNCTION update_wallet_ownership_timestamp()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
CREATE TRIGGER update_wallet_ownership_timestamp_trigger
  BEFORE UPDATE ON public.wallet_ownerships
  FOR EACH ROW EXECUTE FUNCTION update_wallet_ownership_timestamp();
...
-- ============================================================================
-- PHASE 4: Row Level Security (IMPROVED)
-- ============================================================================
  ...
  USING (
    ...
    EXISTS (
      ...
      WHERE wo.wallet_id = id
        ...
        AND (
          (wo.owner_type = 'profile' AND EXISTS (
            SELECT 1 FROM profiles p WHERE p.id = wo.owner_id AND p.user_id = auth.uid()
          )) OR
          (wo.owner_type = 'project' AND EXISTS (
            SELECT 1 FROM projects pr WHERE pr.id = wo.owner_id AND pr.user_id = auth.uid()
          ))
        )
    )
  );
...
-- Wallet Ownerships Policies (IMPROVED VALIDATION)
...
CREATE POLICY "wallet_ownerships_insert_own"
  ...
  WITH CHECK (
    auth.uid() = added_by AND (
      ...
      -- User owns the entity being granted access (with type validation)
...
-- Users can update ownerships they control
CREATE POLICY "wallet_ownerships_update_own"
  ON public.wallet_ownerships FOR UPDATE
  USING (
    (owner_type = 'profile' AND EXISTS (
      SELECT 1 FROM profiles p WHERE p.id = owner_id AND p.user_id = auth.uid()
    )) OR
    (owner_type = 'project' AND EXISTS (
      SELECT 1 FROM projects pr WHERE pr.id = owner_id AND pr.user_id = auth.uid()
    ))
  );
...
-- Wallet Categories Policies (IMPROVED VALIDATION)
...
-- Users can create categories for entities they control (with type validation)
CREATE POLICY "wallet_categories_insert_own"
  ON public.wallet_categories FOR INSERT
  WITH CHECK (
    (entity_type = 'profile' AND EXISTS (
      SELECT 1 FROM profiles p WHERE p.id = entity_id AND p.user_id = auth.uid()
    )) OR
    (entity_type = 'project' AND EXISTS (
      SELECT 1 FROM projects pr WHERE pr.id = entity_id AND pr.user_id = auth.uid()
    ))
  );
...
BEGIN
  ...
  WHERE NOT wd.is_deleted
  GROUP BY wd.id, wd.address_or_xpub, wd.wallet_type, wd.label,
           wd.description, wd.balance_btc, wd.balance_updated_at,
           wo.permission_level, wo.is_active
  ORDER BY wo.is_active DESC, wd.created_at DESC;
END;
$$;
...
BEGIN
  ...
  SELECT
    (SELECT COUNT(*) FROM pg_proc
     WHERE proname IN ('get_entity_wallets', 'validate_wallet_ownership', 'validate_wallet_category')) = 3
  INTO v_functions_exist;

  -- Check triggers exist
  SELECT
    (SELECT COUNT(*) FROM pg_trigger
     WHERE tgname LIKE '%wallet%' AND tgname LIKE '%validate%') >= 2
  INTO v_triggers_exist;

  IF v_tables_exist AND v_functions_exist AND v_triggers_exist THEN
    RAISE NOTICE '✅ SUCCESS: New wallet architecture created (v2 - with referential integrity)';
    RAISE NOTICE '  ✓ Tables: wallet_definitions, wallet_ownerships, wallet_categories';
    RAISE NOTICE '  ✓ Referential Integrity: Triggers for validation and cascade cleanup';
    RAISE NOTICE '  ✓ RLS Policies: Enabled with improved validation';
