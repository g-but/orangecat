      ...
      <Card className="hover:shadow-md transition-shadow cursor-pointer">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            <div
              className={`p-2 rounded-lg ${color.replace('bg-gradient-to-r', 'bg-gradient-to-r').replace('to-', 'to-').split(' ')[0].replace('from-', 'bg-').replace('-500', '-100')}`}
            >
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <div
            className={`p-4 rounded-xl ${color.replace('bg-gradient-to-r', 'bg-gradient-to-r').replace('to-', 'to-').split(' ')[0].replace('from-', 'bg-').replace('-500', '-100')}`}
          >
      ...
      <Card className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-gray-200 hover:border-gray-300 cursor-pointer h-full">
        <CardContent className="p-6 h-full flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <div
              className={`p-3 rounded-xl ${color.replace('bg-gradient-to-r', 'bg-gradient-to-r').replace('to-', 'to-').split(' ')[0].replace('from-', 'bg-').replace('-500', '-100')}`}
            >
