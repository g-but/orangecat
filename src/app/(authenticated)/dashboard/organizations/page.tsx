...
export default function OrganizationsDashboardPage() {
  ...
  return (
    <EntityListPage<Organization>
      ...
      examples={[
        { title: "Red Cross", description: "A global humanitarian organization providing emergency assistance and disaster relief." },
        { title: "Local Food Bank", description: "A community organization distributing food to those in need." },
        { title: "Open Source Project Foundation", description: "A foundation supporting open-source software development." }
      ]}
      renderItem={(o) => (
        <div>
          <div className="font-semibold text-gray-900">{o.name}</div>
          {o.description ? <p className="text-sm text-gray-600 line-clamp-2">{o.description}</p> : null}
          <div className="pt-2">
            <Button href={`/organizations/${o.slug || o.id}`} size="sm" variant="outline">Open</Button>
          </div>
        </div>
      )}
    />
