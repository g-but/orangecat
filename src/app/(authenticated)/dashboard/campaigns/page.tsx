...
export default function ProjectsDashboardPage() {
  ...
  return (
    <EntityListPage<Campaign>
      ...
      examples={[
        { title: "Help John Pay Medical Bills", description: "A personal project to cover unexpected medical expenses through community support." },
        { title: "Food Bank Winter Drive 2025", description: "An organization-led initiative to collect donations for winter supplies and food distribution." },
        { title: "Open Source Project Funding", description: "A project to raise funds for developing and maintaining open-source Bitcoin software." }
      ]}
      renderItem={(c) => (
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className={`text-xs px-2 py-1 rounded-full ${c.isActive ? 'bg-green-100 text-green-700' : c.isPaused ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-700'}`}>
              {c.isActive ? 'Active' : c.isPaused ? 'Paused' : 'Draft'}
            </span>
          </div>
          <div className="text-sm text-gray-700">
            <span>
              {(c.total_funding || 0) > 0 ? `${(c.total_funding || 0)} raised` : 'No funds raised yet'}
              {c.goal_amount ? ` of ${c.goal_amount}` : ''}
            </span>
          </div>
          <div className="pt-2 flex gap-2">
            {c.isActive ? (
              <Button href={`/fund-us/${c.id}`} size="sm" variant="outline">Open</Button>
            ) : (
              <Button href={`/projects/create?draft=${c.id}`} size="sm" variant="outline">Edit</Button>
            )}
          </div>
        </div>
      )}
