...
export async function GET(request: NextRequest) {
  try {
    ...
    const entityCountPromises = ENTITY_TYPES
      .filter(type => type !== 'wallet') // Wallet handled separately
    ...
    const hasWallet = (walletCount ?? 0) > 0 || !!profile.bitcoin_address || !!profile.lightning_address;
    ...
    if (recentProject?.updated_at) {
      ...
      const now = new Date();
      daysSinceLastActivity = Math.floor((now.getTime() - lastUpdate.getTime()) / (1000 * 60 * 60 * 24));
    }
