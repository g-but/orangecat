...
function base() { return process.env.EXPLORER_BASE_URL || 'https://mempool.space' }
async function j(url: string) { const r = await fetch(url, { cache: 'no-store' as any }); if (!r.ok) throw new Error(`${r.status} ${url}`); return r.json() }
function txUrl(txid: string) { return `${base()}/tx/${txid}` }
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    function mapTx(tx: any) {
      ...
      return { txid: tx.txid, time: tx.status?.block_time || tx.received_at || null, amount_sats: Math.abs(d), direction: d >= 0 ? 'in' : 'out', confirmations: confs, explorer_url: txUrl(tx.txid) }
    }
    const txs = [...(memTxs || []), ...(chainTxs || [])].slice(0, 20).map(mapTx)
    const payload = { address: addr, explorer_base: b, received_sats, spent_sats, balance_sats, unconfirmed_sats, txs, last_updated: new Date().toISOString() }
