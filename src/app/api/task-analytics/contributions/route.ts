...
export const GET = withAuth(async (request: AuthenticatedRequest) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id, completed_by, completed_at, duration_minutes, task:tasks!task_completions_task_id_fkey(id, title, category)')
    ...
    if (uniqueUserIds.length > 0) {
      const { data: profiles } = await supabase.from(DATABASE_TABLES.PROFILES).select('id, username, display_name, avatar_url').in('id', uniqueUserIds);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return apiSuccess({
      ...
      summary: {
        ...
        averagePerPerson: contributions.length > 0 ? Math.round(totalCompletions / contributions.length) : 0,
