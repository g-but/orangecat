...
export const POST = compose(withRateLimit('write'))(async (request: NextRequest) => {
  ...
  try {
    ...
    if (entity.contributions && entity.contributions.length > 0) {
      const userContributions = entity.contributions.filter((c: any) => c.user_id === user.id);
