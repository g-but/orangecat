...
import { apiSuccess, apiUnauthorized, handleApiError, apiRateLimited, apiValidationError } from '@/lib/api/standardResponse';
...
export const POST = compose(withRequestId(), withZodBody(projectSchema))(async (request: NextRequest, ctx) => {
  try {
    ...
    if (error instanceof Error && (error as any).name === 'ZodError') {
      ...
      return apiValidationError('Invalid project data', { details: zodError.errors || zodError.message })
    }
    return handleApiError(error)
  }
})
