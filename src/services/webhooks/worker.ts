 ...
 *
 * Created: 2026-06-03
 * Extracted from app/api/cron/webhook-worker/route.ts (pure move) — the
 * route is a thin wrapper: cron-secret check + this batch + HTTP response.
 */
...
export const LOG_SOURCE = 'CronWebhookWorker';
const BATCH_SIZE = 20;
...
export interface WebhookWorkerResult {
  processed: number;
  summary?: Record<DeliveryOutcome['outcome'], number>;
  ranAt: string;
}

/** Run one webhook delivery batch. Throws on selection failure. */
export async function runWebhookWorkerBatch(): Promise<WebhookWorkerResult> {
  const due = await pickDueDeliveries(BATCH_SIZE);
  if (due.length === 0) {
    return { processed: 0, ranAt: new Date().toISOString() };
  }
  ...
  const summary = outcomes.reduce(
    ...
    { delivered: 0, retry: 0, failed: 0, skipped: 0 } as Record<DeliveryOutcome['outcome'], number>
  );
