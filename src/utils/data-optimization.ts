...
export class InitiativesDataLoader {
  ...
  async getInitiativesByCategory(category: string): Promise<any[]> {
    ...
    return allInitiatives.filter(
      initiative => initiative.category?.toLowerCase() === category.toLowerCase()
    );
  }
...
export class DashboardConfigLoader {
  ...
  async getConfigsByRole(role: string = 'user'): Promise<any[]> {
    ...
    const roleConfigs = allConfigs.filter(
      config => config.allowedRoles?.includes(role) || !config.allowedRoles
    );
  ...
  private async loadAllConfigs(): Promise<any[]> {
    ...
    return [projectsConfig, fundraisingConfig];
  }
 ...
 */
export function useVirtualScroll<T>(data: T[], itemHeight: number, containerHeight: number) {
